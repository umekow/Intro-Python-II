...
room['dungeon'].items = [Item('skull', 'a wolf\'s skull'), Item(
    'candlestick', 'a candlestick that is about to give out')]
room['outside'].items = [Item('vase', 'a dang vase'), Item('gun', 'a pistol with no bullets'), Item(
    'egg', 'Did it come first?'), Item('rose', 'a dying but beautiful rose')]
room['foyer'].items = [Item('umbrella', 'its broken!'), Item(
    'basket', 'who would want this?')]
room['treasure'].items = [Item('empty chest', 'where\'s the cash, man?'), Item(
    'ruby', 'Rubies are a girl\'s bestfriend...wait')]
room['narrow'].items = [Item('sword', 'a rusty sword'),  Item(
    'elf', 'He thinks anything you say is offensive towards elves')]
#
...
while cmd != 'q':
    ...
    print(f'Current room: {user_player.current_room.name}, {user_player.current_room.description}\nItems in this room:',
          user_player.current_room.get_items())
    ...
    if cmd == 'q':
        ...
        if user_player.current_room.name == 'Dungeon' and cmd == 's':
            ...
            if 'candlestick' in user_player.get_items():
                ...
                print(
                    "It is too dark in there! (go back to find something to light your path)")
        else:
            user_player.travel(cmd)
